...
class ControllerProductCategory extends Controller {
	public function index() {
		...
		if ($category_info) {
			$meta_title = preg_replace($patterns, $replacements, $category_info['meta_title']);
			$meta_description = preg_replace($patterns, $replacements, $category_info['meta_description']);
			$meta_keyword = preg_replace($patterns, $replacements, $category_info['meta_keyword']);
			...
			$description = preg_replace($patterns, $replacements, $category_info['description']);
			...
			foreach ($results as $result) {
				...
				$product_description = preg_replace($patterns, $replacements, $result['description']);

				$data['products'][] = array(
					'product_id'  => $result['product_id'],
					'thumb'       => $image,
					'name'        => $result['name'],
					'description' => utf8_substr(strip_tags(html_entity_decode($product_description, ENT_QUOTES, 'UTF-8')), 0, $this->config->get($this->config->get('config_theme') . '_product_description_length')) . '..',
					'price'       => $price,
					'special'     => $special,
					'tax'         => $tax,
