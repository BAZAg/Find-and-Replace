...
class ModelSettingStore extends Model {
	...
	public function setProductsAndCategoryToStore($store_id) {
		$query = $this->db->query("SELECT category_id FROM " . DB_PREFIX . "category");
		foreach ($query->rows as $result) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "category_to_store SET category_id = '" . (int)$result['category_id'] . "', store_id = '" . (int)$store_id . "'");
		}

		$query = $this->db->query("SELECT product_id FROM " . DB_PREFIX . "product");
		foreach ($query->rows as $result) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "product_to_store SET product_id = '" . (int)$result['product_id'] . "', store_id = '" . (int)$store_id . "'");
		}
	}
}